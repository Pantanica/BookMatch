...
app.post('/api/comentarios', verifyToken, async (req, res) => {
    ...
    const autor_id = req.user.userId; // ID del autor se obtiene del token del usuario autenticado

    try {
        const nuevoComentario = await db.query(
            'INSERT INTO comentarios_usuarios (usuario_id, autor_id, calificacion, comentario) VALUES ($1, $2, $3, $4) RETURNING *',
            [usuario_id, autor_id, calificacion, comentario]
        );
...
app.get('/api/comentarios/:usuario_id', async (req, res) => {
    const { usuario_id } = req.params;

    try {
        const comentarios = await db.query(
            `SELECT c.*, u.nombres AS autor_nombre 
...
app.get('/api/mis-comentarios', verifyToken, async (req, res) => {
    const autorId = req.user.userId; // Este es el usuario autenticado
    try {
        const query = `
            SELECT c.comentario, c.calificacion, c.fecha, u.nombres AS usuario_nombre
            FROM comentarios_usuarios c
            INNER JOIN perfil_usuario u ON u.id = c.usuario_id
            WHERE c.autor_id = $1
        ...
        console.error("Error al obtener comentarios hechos por el usuario:", error);
        res.status(500).json({ error: "No se pudieron obtener los comentarios." });
    }
});


// Dentro de tu componente principal donde configuras las rutas
<Router>
    <Routes>
        <Route path="/userProfile" element={<UserProfilePage />} />
        <Route path="/mis-comentarios" element={<MyCommentsPage />} />
        {/* Otras rutas */}
    </Routes>
</Router>